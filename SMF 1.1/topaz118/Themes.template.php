...
function template_set_settings()
{
	...
	foreach ($context['settings'] as $setting)
	{
		...
		if(isset($setting['bar']))
			echo '<div class="catbg" style="height: 10px;"></div><br />';
		if ($setting['type'] == 'checkbox')
			echo '
					<input type="hidden" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" value="0" />
		...
		elseif ($setting['type'] == 'menulist' && function_exists('TP_getallmenus'))
		{
			$menus= TP_getallmenus();
			echo '
			...
			{
				echo '
							<option value="', $menu['id'], '"', $menu['id'] == $setting['value'] ? ' selected="selected"' : '', '>', $menu['name'], '</option>';
			}
		...
		elseif ($setting['type'] == 'list')
		{
			if (isset($setting['subtype']) && $setting['subtype'] == 'radio')
			{
				foreach ($setting['options'] as $value => $label)
				{
					echo '
					<div class="bordercolor" style="padding: 1px; margin: 2px; float: left; width: 45%;">
						<div class="windowbg" style="padding: 5px; overflow: hidden;">
							<input value="', $value, '"', $value == $setting['value'] ? ' checked="checked"' : '', ' type="radio" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '"> ', $label;

					if(isset($setting['showimage']))
							echo '
							<br><br><img align="texttop" src="' . $settings['images_url'] . '/theme/'.$value.'.png" alt="" />';
					elseif(isset($setting['showcolor']))
							echo '
							<div style="background: '.$value.'; float: right; width: 50%; height: 20px;">&nbsp;</div>';
					else
						echo '<br>';
					echo '
						</div>
		...
		elseif ($setting['type'] == 'textarea')
		{
			echo '
					<label for="', $setting['id'], '">', $setting['label'], '</label>
							<textarea style="width: 90%; height: 200px; " name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '">', $setting['value'], '</textarea>';
		}
		else
			echo '
					<label for="', $setting['id'], '">', $setting['label'], '</label>
