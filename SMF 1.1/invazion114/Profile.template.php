...
function template_summary()
{
	...
	// new profile?
	if(empty($settings['show_oldprofile']))
	{
		...
		echo '
		<div id="summarytabs">
			<a href="' . $scripturl . '?action=profile;u=' .$context['member']['id']  . '"' , !$showold ? ' style="font-weight: bold;"' : '' , '>Compact</a> | 
			<a href="' . $scripturl . '?action=profile;u=' .$context['member']['id']  . ';fullprofile"' , $showold ? ' style="font-weight: bold;"' : '' , '>Full</a>
		</div>'; 
		...
		<div id="summary_profile">
			...
			if ($context['member']['email_public'])
				echo '
				<a href="mailto:', $context['member']['email'], '"><img src="' , $settings['images_url'] , '/email_sm.gif" alt="', $context['member']['email'], '" /></a>';
			...
			if(!empty($context['member']['options']['invazionskype']))
				echo '
				<a href="callto://', $context['member']['options']['invazionskype'], '"><img src="' , $settings['images_url'] , '/skype.png" alt="', $context['member']['options']['invazionskype'], '" /></a>';
			...
			for($a=1 ; $a<5 ; $a++)
			{
				if(!empty($context['member']['options']['userpic'.$a]))
				{
					echo '<div class="userpics"><img src="' , strip_tags($context['member']['options']['userpic'.$a]) , '" alt="" /></div>';
				}
			}
			echo '
			</fieldset>
			<div class="avatar_member">', $context['member']['avatar']['image'], '</div>
			<h2>' , $context['member']['name'] , ' <small>( ' . $context['member']['online']['text'] . ' )</small></h2>
		
			<p class="blurb">', $context['member']['blurb'], '</p>
			<p class="infoprof"><strong>', (!empty($context['member']['group']) ? $context['member']['group'] : $context['member']['post_group']), '</strong> , 
				' , $txt['lastLoggedIn'] , ' ', $context['member']['last_login'], '<em>
				' , $context['member']['posts'], ' ' , $txt[21] , ' </em> (', $context['member']['posts_per_day'], ' ', $txt['posts_per_day'], ') since ', $context['member']['registered'], '
			</p>
			' , !empty($context['member']['options']['userbio']) ? '<p class="userbio">' . parse_bbc($context['member']['options']['userbio']) . '</p>' : '' , '
		
			</div>';
		}
		else
			echo '<div id="oldprofile">' , template_summary_old() ,'</div>';
	}
	else
		template_summary_old();
}

// "old" profile summary
function template_summary_old()
{
	global $context, $settings, $options, $scripturl, $modSettings, $txt;
							...
							<tr>
								<td colspan="2">
									<table width="100%" cellspacing="0" cellpadding="3">
										<tr><td colspan="2" class="catbg">Custom options</td></tr>';

	if(empty($settings['disallow_categorytabs']))
		echo '
										<tr>
											<td colspan="2">
												<input type="hidden" name="default_options[use_categorytabs]" value="0" />
												<label for="use_categorytabs"><input type="checkbox" name="default_options[use_categorytabs]" id="use_categorytabs" value="1"', !empty($context['member']['options']['use_categorytabs']) ? ' checked="checked"' : '', ' class="check" />Show boards and categories as tabs?</label>
											</td>
										</tr>';
	if(empty($settings['disallow_userpics']))
		echo '
										<tr><td colspan="2" class="catbg">Bio and Gallery</td></tr>
										<tr>
											<td colspan="2">
												Please use only direct links to images, for example from picture sharing sites like <a href="http://www.flickr.com/" target="_blank">Flickr</a> or <a href="http://photobucket.com/" target="_blank">Photobucket</a>.
												</td>
										</tr>
										<tr>
											<td colspan="2">
												<input type="hidden" name="default_options[userpic1]" value="0" />
												<label for="userpic1"><input size="80" type="text" name="default_options[userpic1]" id="userpic1" value="', !empty($context['member']['options']['userpic1']) ?  $context['member']['options']['userpic1'] : '', '"  /> Path to User picture 1</label>
											</td>
										</tr>
										<tr>
											<td colspan="2">
												<input type="hidden" name="default_options[userpic2]" value="0" />
												<label for="userpic2"><input size="80" type="text" name="default_options[userpic2]" id="userpic2" value="', !empty($context['member']['options']['userpic2']) ?  $context['member']['options']['userpic2'] : '', '"  /> Path to User picture 2</label>
											</td>
										</tr>
										<tr>
											<td colspan="2">
												<input type="hidden" name="default_options[userpic3]" value="0" />
												<label for="userpic3"><input size="80" type="text" name="default_options[userpic3]" id="userpic3" value="', !empty($context['member']['options']['userpic3']) ?  $context['member']['options']['userpic3'] : '', '"  /> Path to User picture 3</label>
											</td>
										</tr>
										<tr>
											<td colspan="2">
												<input type="hidden" name="default_options[userpic4]" value="0" />
												<label for="userpic4"><input size="80" type="text" name="default_options[userpic4]" id="userpic4" value="', !empty($context['member']['options']['userpic4']) ?  $context['member']['options']['userpic4'] : '', '"  /> Path to User picture 4</label>
											</td>
										</tr>
										<tr>
											<td colspan="2">
												<input type="hidden" name="default_options[userbio]" value="0" />
												<label for="userbio"><textarea cols="60" rows="8" style="width: 96%; height: 300px;" name="default_options[userbio]" id="userbio">', !empty($context['member']['options']['userbio']) ?  $context['member']['options']['userbio'] : '', '</textarea><br />Personal description</label>
											</td>
										</tr>
										<tr><td colspan="2">';
										for($a=1 ; $a<5; $a++)
										{
											if(!empty($context['member']['options']['userpic'.$a]))
												echo '<img class="userpics" src="' , $context['member']['options']['userpic'.$a] , '" alt="" />';
										}
										echo '
										</td></tr>';
	echo '
										<tr>
											<td colspan="2">
												<input type="hidden" name="default_options[invazionskype]" value="0" />
												<label for="invazionskype"><input size="80" type="text" name="default_options[invazionskype]" id="invazionskype" value="', !empty($context['member']['options']['invazionskype']) ?  $context['member']['options']['invazionskype'] : '', '"  /> <img src="'.$settings['images_url'].'/skype.png" alt="SKYPE" /> SKYPE</label>
											</td>
										</tr>
										<tr><td colspan="2" class="catbg">Normal options</td></tr>
										</tr>
										<tr>
											<td colspan="2">
