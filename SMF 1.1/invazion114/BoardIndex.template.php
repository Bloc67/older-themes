...
<div class="tborder" style="margin-bottom: 2ex;">
	...
	// do we render tabs or not?
	if(!empty($options['use_categorytabs']) && empty($settings['disallow_categorytabs']))
		renderBoardsTabs();
	else
		renderBoards();

	// Recent topics
	if(!empty($settings['show_recenttopics']))
	{
		global $boarddir;
	...
	echo '
		<h3 class="titlebg" style="padding: 4px; text-align: center;">Recent Topics</h3>
		<ul id="recent_top">';
	foreach ($what as $rec)
		echo '
			<li><div style="float: left; margin-right: 4px;">' , $rec['icon'] , '</div>' , $rec['link'] , !$rec['new'] ? '&nbsp;<a href="' . $rec['href'] . '"><img src="' . $settings['images_url'] . '/' . $context['user']['language'] . '/new.gif" alt="" /></a>' : '' , ' ', $txt[525], ' ' , $rec['poster']['link'] , ' ', $txt[107], ' ' , $rec['time'] , '<br /><span class="smalltext">', $txt['smf88'], ' ' ,  $rec['board']['link'] , '</span></li>';
				
	echo '
		</ul>';
	}
	// how about Info center? is it off?
	if(empty($settings['off_infocenter']))
		renderInfocenter();

}

// the normal view on boards
function renderBoards()
{
	...
	{
		$first=true;
		echo '<h3 class="catbg" style="margin: 0; padding: 6px;">';
		...
		if (!$category['is_collapsed'])
		{
			...
			foreach ($category['boards'] as $board)
			{
				...
				if (!empty($board['children']))
				{
					...
					echo '
			<tr>
				<td colspan="5" class="innerpads windowbg3">
			...
			if (!empty($settings['use_ads']) && function_exists("show_category"))
			{
...
<h3 class="titlebg" style="padding: 4px; margin: 0; text-align: center;">' , $txt[685] , '</h3>
<table border="0" width="100%" cellspacing="1" cellpadding="4">';
...
function renderBoardstabs()
{
	...
	foreach ($context['categories'] as $category)
	{
		...
		echo '
					<a id="cattab' . $category['id'] . '" ' , $options['showcat']==$category['id'] ? 'class="chosen"' : '' , ' href="#" onclick="showcat('.$category['id'].')">', $category['name'], '</a>
				</li>';
	}
	echo '</ul>';

	$first=true;
	// then again for the boards
	foreach ($context['categories'] as $category)
	{
			echo '
	<div class="category" id="cat'.$category['id'].'" ' , $options['showcat']==$category['id'] ? '' : 'style="display: none;"' , '>
		<table border="0" width="100%" cellspacing="1" cellpadding="5" class="bordercolor" style="margin-top: 1px;">
			...
			<tr>
				...
				// If the board is new, show a strong indicator.
				if ($board['new'])
					echo '<img src="', $settings['images_url'], '/img/on.png" alt="', $txt[333], '" title="', $txt[333], '" />';
				// This board doesn't have new posts, but its children do.
				elseif ($board['children_new'])
					echo '<img src="', $settings['images_url'], '/img/on2.png" alt="', $txt[333], '" title="', $txt[333], '" />';
				// No new posts at all! The agony!!
				else
					echo '<img src="', $settings['images_url'], '/img/off.png" alt="', $txt[334], '" title="', $txt[334], '" />';
	...
	<table border="0" width="100%" cellspacing="0" cellpadding="10">
		<tr>
			<td align="', !$context['right_to_left'] ? 'left' : 'right', '" class="smalltext">
				<img src="' . $settings['images_url'] . '/img/on.png" width="20" height="18" alt="" align="middle" /> ', $txt[333], '
				<img src="' . $settings['images_url'] . '/img/off.png" width="20" height="18" alt="" align="middle" style="margin-left: 4ex;" /> ', $txt[334], '
			</td>
