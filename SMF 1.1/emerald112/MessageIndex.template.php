...
function template_main()
{
	...
	{
		echo '
	<h2 class="catbg">', $txt['parent_boards'], '</h2>';
	...
	{
		echo '
		<div class="center_descript">', $context['description'], '</div>';
	}
	...
	<form action="', $scripturl, '?action=quickmod;board=', $context['current_board'], '.', $context['start'], '" method="post" accept-charset="', $context['character_set'], '" name="quickModForm" id="quickModForm" style="margin: 0;">';
		...
		{
			echo '
						<td width="3%" colspan="2" class="catbg3"></td>
		...
		foreach ($context['topics'] as $topic)
		{
			...
			echo '
					<tr class="'.$class.'">
						...
						<td valign="middle" ', (!empty($topic['quick_mod']['remove']) ? 'id="topic_' . $topic['first_post']['id'] . '" onmouseout="mouse_on_div = 0;" onmouseover="mouse_on_div = 1;" ondblclick="modify_topic(\'' . $topic['id'] . '\', \'' . $topic['first_post']['id'] . '\', \'' . $context['session_id'] . '\');"' : ''), '>';
		...
		echo template_button_strip_tp($normal_buttons, 'top');
	}


	// Show breadcrumbs at the bottom too?
	echo '
	<div style="padding: 1em 0 1em 0;">', theme_linktree(), '</div>';
