	...
	foreach ($context['settings'] as $setting)
	{
		...
		elseif ($setting['type'] == 'radio')
		{
			if(!empty($setting['thumbs']))
				$imgpath = $settings['images_url'] . '/' . $setting['thumbs'];

			foreach ($setting['options'] as $value => $label)
			{
				echo '
					<label for="', $setting['id'], '">', $setting['label'], '</label>
					<input value="', $value, '"', $value == $setting['value'] ? ' selected="selected"' : '', ' name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" type="radio" id="', $setting['id'], '">', $label, '<br />';

				if(!empty($imgpath))
					echo '<br /><img src="' . $imgpath . '/' . $setting['value'] . '" alt="" /><br />';
			}
		}
		elseif ($setting['type'] == 'textarea')
		{
			echo '
					<label for="', $setting['id'], '">', $setting['label'], '</label>
					<textarea style="width: 100%; height: 300px;" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '"> ', $setting['value'], '</textarea>';
		}
		else
			echo '
					<label for="', $setting['id'], '">', $setting['label'], '</label>
