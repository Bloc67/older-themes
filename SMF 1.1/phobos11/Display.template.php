...
<a name="msg', $context['first_message'], '"></a>', $context['first_new_message'] ? '<a name="new"></a>' : '';
	...
	if ($context['is_poll'])
	{
		echo $settings['titlebg1']. '<div style="background: url(', $settings['images_url'], '/topic/', $context['poll']['is_locked'] ? 'normal_poll_locked' : 'normal_poll', '.gif) 0% 50% no-repeat; padding-left: 28px;">', $txt['smf43'], '</div>' , $settings['titlebg2']; 

		echo $settings['greybox1'] . '
<table cellpadding="3" cellspacing="0" border="0" width="100%" class="tborder" style="padding-top: 0;">
	<tr>
	...
	{
		echo $settings['titlebg1'], $txt['calendar_linked_events'] , $settings['titlebg2'];
		echo $settings['greybox1'].'
<table cellpadding="3" cellspacing="0" border="0" width="100%" class="tborder" style="padding-top: 0; ">
		<tr>
...
<table cellpadding="0" cellspacing="0" border="0" width="100%" class="bordercolor">';
	...
	<tr><td style="padding: 1px 1px 0 1px;">';
		...
		if (!$message['member']['is_guest'])
		{
			...
			if(!empty($settings['use_viswarn'])){								
				if (isset($message['member']['warning']['link']) && $message['member']['warning']['link'] != '')
					echo'</div>' , $message['member']['warning']['link'], '<div align="center">';
			}
        ...
        if(function_exists("arcade_champs_post_profile"))
		...
		echo '
						...
						<td width="70%" valign="top" height="100%">
							<table width="100%" border="0" style="margin-bottom: 10px;"><tr>
								<td width="20" valign="middle" style="padding-left: 20px;"><a href="', $message['href'], '"><img src="', $message['icon_url'] . '" alt="" border="0" /></a></td>
								<td valign="middle">
		...
		echo '
								...
								<td align="', !$context['right_to_left'] ? 'right' : 'left', '" valign="bottom" height="20" style="padding-right: 20px;font-size: smaller;">';

		// Can they reply? Have they turned on quick reply?
		if ($context['can_reply'] && !empty($options['display_quick_reply']))
			echo '
					<a href="', $scripturl, '?action=post;quote=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';num_replies=', $context['num_replies'], ';sesc=', $context['session_id'], '" onclick="doQuote(', $message['id'], ', \'', $context['session_id'], '\'); return false;"><img src="'.$settings['images_url'].'/buttons/quote.gif" alt="" title="'.$txt[145].'" /></a>';

		// So... quick reply is off, but they *can* reply?
		elseif ($context['can_reply'])
			echo '
					<a href="', $scripturl, '?action=post;quote=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';num_replies=', $context['num_replies'], ';sesc=', $context['session_id'], '"><img src="'.$settings['images_url'].'/buttons/reply.gif" alt="'.$txt[145].'" title="'.$txt[145].'" /></a>';

		// Can the user modify the contents of this post?
		if ($message['can_modify'])
			echo '
					<a href="', $scripturl, '?action=post;msg=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';sesc=', $context['session_id'], '"><img src="'.$settings['images_url'].'/buttons/modify.gif" alt="'.$txt[66].'" title="'.$txt[66].'" /></a>';

		// How about... even... remove it entirely?!
		if ($message['can_remove'])
			echo '
					<a href="', $scripturl, '?action=deletemsg;topic=', $context['current_topic'], '.', $context['start'], ';msg=', $message['id'], ';sesc=', $context['session_id'], '" onclick="return confirm(\'', $txt[154], '?\');"><img src="'.$settings['images_url'].'/buttons/delete.gif" alt="'.$txt[121].'" title="'.$txt[121].'" /></a>';
		...
		if(!empty($settings['use_viswarn']))
		{
			if ($context['can_warn'])
				echo '
					<a href="', $scripturl, '?action=warnadd;topic=', $context['current_topic'], ';start=', $context['start'], ';user=', $message['member']['id'], ';msg=', $message['id'], '"><img src="'.$settings['images_url'].'/buttons/warn.gif" alt="Warn this user" title="Warn this user" /></a>';
		}
		
		// What about splitting it off the rest of the topic?
		if ($context['can_split'])
			echo '
					<a href="', $scripturl, '?action=splittopics;topic=', $context['current_topic'], '.0;at=', $message['id'], '"><img src="'.$settings['images_url'].'/buttons/split.gif" alt="'.$txt['smf251'].'" title="'.$txt['smf251'].'" /></a>';
		...
		echo '
							...
							' . $settings['greybox1'] . '<div style="padding: 10px;">
							<div class="post"', $message['can_modify'] ? ' id="msg_' . $message['id'] . '"' : '', '>', $message['body'], '', $message['can_modify'] ? '
							<img src="' . $settings['images_url'] . '/icons/modify_inline.gif" alt="" align="right" id="modify_button_' . $message['id'] . '" style="cursor: pointer; display: none; margin: 15px;" onclick="modify_msg(\'' . $message['id'] . '\', \'' . $context['session_id'] . '\')" />' : '' , '
							</div></div>' . $settings['greybox2'] . '
						</td>
					</tr>';
		...
		if ($context['can_report_moderator'])
			echo '
									<a href="', $scripturl, '?action=reporttm;topic=', $context['current_topic'], '.', $message['counter'], ';msg=', $message['id'], '"><img src="', $settings['images_url'], '/ip.gif" alt="" border="0" /></a>';
...
<table border="0" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 1ex;">
	...
	echo $settings['titlebg1']. '
<div class="tborder"><div align="', !$context['right_to_left'] ? 'right' : 'left', '">
	<form action="', $scripturl, '" method="get" accept-charset="', $context['character_set'], '" style="padding:0; margin: 0;">
		...
		<select style="padding: 0; margin: 0; font-size: 10px;" name="jumpto" id="jumpto" onchange="if (this.selectedIndex > 0 &amp;&amp; this.options[this.selectedIndex].value) window.location.href = smf_scripturl + this.options[this.selectedIndex].value.substr(smf_scripturl.indexOf(\'?\') == -1 || this.options[this.selectedIndex].value.substr(0, 1) != \'?\' ? 0 : 1);">
	...
	echo '
		...
		<input  style="padding: 0; margin: 0;"  type="button" value="', $txt[161], '" onclick="if (this.form.jumpto.options[this.form.jumpto.selectedIndex].value) window.location.href = \'', $scripturl, '\' + this.form.jumpto.options[this.form.jumpto.selectedIndex].value;" />&nbsp;&nbsp;&nbsp;
	</form>
</div></div>' . $settings['titlebg2'];
