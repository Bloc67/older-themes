...
function template_set_settings()
{
	...
	foreach ($context['settings'] as $setting)
	{
		...
		elseif($setting['type'] == 'textarea')
			echo '
					<label for="', $setting['id'], '">', $setting['label'], '</label><br />
					<textarea style="width: 90%; height: 150px;" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '">', $setting['value'], '</textarea>';
		else
			echo '
					<label for="', $setting['id'], '">', $setting['label'], '</label>
