...
function template_set_settings()
{
	...
	foreach ($context['settings'] as $setting)
	{
		...
		{
			echo '
					<textarea rows="4" cols="60" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '">', (empty($setting['value']) && isset($setting['empty'])) ? $setting['empty'] : $setting['value'], '</textarea>
