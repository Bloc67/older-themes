...
function template_set_settings()
{
	...
	foreach ($context['settings'] as $setting)
	{
		...
		{
			echo '
					<textarea rows="4" cols="40" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '">', (empty($setting['value']) && isset($setting['empty'])) ? $setting['empty'] : $setting['value'], '</textarea>
					<br /><label for="', $setting['id'], '">', $setting['label'], '</label>';
		}
		elseif ($setting['type'] == 'paragraph')
			echo '
					<div>', (empty($setting['text'])) ? '' : $setting['text'], '</div>';
		else
			echo '
					<label for="', $setting['id'], '">', $setting['label'], '</label>
