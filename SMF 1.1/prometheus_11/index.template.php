...
function template_init()
{
	...
	if(empty($settings['custom_pages']))
		$settings['custom_pages']='homepage,Home,page|about,Custom,page|http.//www.blocweb.net,TP,link';
...
<script language="JavaScript" type="text/javascript"><!-- // --><![CDATA[
		...
		{
			setInnerHTML(document.getElementById("showtext") , text);
		}
		// ]]></script>
</head>
<body>';
...
<div id="mainframe">
	<div id="topframe">
	' , topmenu() ,'
		<h1 id="forum"><a href="',$scripturl,'">'.$context['forum_name'].'</a></h1>
		<h2 id="greeting">';
	...
	<div id="content">
		<div id="innerborder">
		<div id="showtext_outer"><div id="showtext">
		Go to
		</div></div>';	
	template_menu();
   ...
   echo '</td>';
	...
	<div id="footerarea" style="text-align: center; padding-bottom: 1ex;', $context['browser']['needs_size_fix'] && !$context['browser']['is_ie6'] ? ' width: 100%;' : '', '">
								<div>
									...
									<strong>Prometheus</strong> design by <a href="http.//www.blocweb.net" target="_blank">BlocWeb</a> |
...
function template_menu()
{
	...
	// Show the start of the tab section.
		echo '
			<div id="menudiv">
				<ul id="menuhoriz">';	
	echo '<li><a onmouseover="chtext(\''.$txt[103].'\');"  id="homebtn" ' , $current_action=='home' ? ' class="chosen"' : '' , ' href="', $scripturl, '"><span>' , $txt[103] , '</span></a></li>';

		// TP
		if (!empty($settings['use_tp']))
			echo '<li><a onmouseover="chtext(\''.$txt['tp-forum'].'\');" id="forumbtn" ' , $current_action=='forum' ? ' class="chosen"' : '' , ' href="', $scripturl, '?action=forum"><span>' , $txt['tp-forum'] , '</span></a></li>';


	if ($context['allow_calendar'])
		echo '<li><a onmouseover="chtext(\''.$txt['calendar24'].'\');"  id="calendarbtn" ' , $current_action=='calendar' ? ' class="chosen"' : '' , ' href="', $scripturl, '?action=calendar"><span>' , $txt['calendar24'] , '</span></a></li>';
	if ($context['allow_admin'])
		echo '<li><a onmouseover="chtext(\''.$txt[2].'\');"  id="adminbtn" ' , $current_action=='admin' ? ' class="chosen"' : '' , ' href="', $scripturl, '?action=admin"><span>' , $txt[2] , '</span></a></li>';	
	if ($context['allow_edit_profile'])
		echo '<li><a onmouseover="chtext(\''.$txt[79].'\');"  id="profilebtn" ' , $current_action=='profile' ? ' class="chosen"' : '' , ' href="', $scripturl, '?action=profile"><span>' , $txt[79] , '</span></a></li>';

	if ($context['user']['is_logged'] && $context['allow_pm'])
		echo '<li  onmouseover="chtext(\''.$txt['pm_short'].'\');" ><a id="pmbtn" ' , $current_action=='pm' ? ' class="chosen"' : '' , ' href="', $scripturl, '?action=pm"><span>' , $txt['pm_short'] , ' ', $context['user']['unread_messages'] > 0 ? '[<strong>'. $context['user']['unread_messages'] . '</strong>]' : '' , '</span></a></li>';


	if ($context['user']['is_logged'])
		echo '<li><a onmouseover="chtext(\''.$txt[108].'\');"  id="logoutbtn"  href="', $scripturl, '?action=logout;sesc=', $context['session_id'], '"><span>' , $txt[108] , '</span></a></li>';
	if ($context['user']['is_guest'])
		echo '<li><a onmouseover="chtext(\''.$txt[34].'\');"  id="loginbtn" ' , $current_action=='login' ? ' class="chosen"' : '' , ' href="', $scripturl, '?action=login"><span>' , $txt[34] , '</span></a></li>';	

		echo '
					<li><a onmouseover="chtext(\''.$txt[119].'\');" id="helpbtn" ' , $current_action=='help' ? ' class="chosen"' : '' , ' href="', $scripturl, '?action=help"><span>' , $txt[119] , '</span></a></li>';
		if ($context['allow_search'])
						echo '<li><a onmouseover="chtext(\''.$txt[182].'\');"  id="searchbtn" ' , $current_action=='search' ? ' class="chosen"' : '' , ' href="', $scripturl, '?action=search"><span>Search</span></a></li>';
			
		if ($context['user']['is_guest'])
			echo '<li><a onmouseover="chtext(\''.$txt[97].'\');"  id="registerbtn" ' , $current_action=='register' ? ' class="chosen"' : '' , ' href="', $scripturl, '?action=register"><span>' , $txt[97] , '</span></a></li>';	

		// SMF Arcade
		if (!empty($settings['use_arcade']))
			echo '<li><a onmouseover="chtext(\''.$txt['arcade'].'\');"  id="arcadebtn" ' , $current_action=='arcade' ? ' class="chosen"' : '' , ' href="', $scripturl, '?action=arcade"><span>' , $txt['arcade'] , '</span></a></li>';
		// SMF Gallery
		if (!empty($settings['use_smfgallery']))
			echo '<li><a onmouseover="chtext(\''.$txt['smfgallery_menu'].'\');"  id="gallerybtn" ' , $current_action=='gallery' ? ' class="chosen"' : '' , ' href="', $scripturl, '?action=gallery"><span>' , $txt['smfgallery_menu'] , '</span></a></li>';
		// SMFshop 
		if (!empty($settings['use_shop']))
			echo '<li><a onmouseover="chtext(\'Shop\');"  id="shopbtn" ' , $current_action=='shop' ? ' class="chosen"' : '' , ' href="', $scripturl, '?action=shop"><span>Shop</span></a></li>';
