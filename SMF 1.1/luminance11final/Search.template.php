...
function template_main()
{
	...
	else
	{
		echo '
					...
					<table cellpadding="1" cellspacing="3" border="0" width="100%">
						<tr>
							...
							<td colspan="3" width="100%" class="modbutton">
								...
								<br /><b>', $txt[583], ':</b><br />
								<input style="margin-top: 5px;" type="text" name="userspec" value="', empty($context['search_params']['userspec']) ? '*' : $context['search_params']['userspec'], '" size="40" />
							</td>
						</tr>';
		if (empty($modSettings['search_simple_fulltext']))
			echo '
		...
		echo '
						<tr>
							<td colspan="3" class="viewbutton"><br />
								<div style="text-align: left; margin-right: 2ex;">
							...
							<td style="padding-top: 2ex;" colspan="3" class="viewbutton"><b>', $txt['search_order'], ':</b><br />
								<select name="sort" style="margin-top: 5px;">
		...
		{
			echo '
					<fieldset class="modbutton" style="padding: 10px; margin-left: 5px; margin-right: 5px;">
	...
	else
	{
		...
		while ($topic = $context['get_topics']())
		{
			foreach ($topic['matches'] as $message)
			{
				...
				if ($topic['can_reply'])
				{
					$buttonArray[] = '<a href="' . $scripturl . '?action=post;topic=' . $topic['id'] . '.' . $message['start'] . '">' . ($settings['use_image_buttons'] ? '<img src="' . $settings['images_url'] . '/' . $context['user']['language'] . '/reply_sm.gif" alt="' . $txt[146] . '" />' : $txt[146]) . '</a>';
					$buttonArray[] = '<a href="' . $scripturl . '?action=post;topic=' . $topic['id'] . '.0;quote=' . $message['id'] . '/' . $message['start'] . ';sesc=' . $context['session_id'] . '">' . ($settings['use_image_buttons'] ? '<img src="' . $settings['images_url'] . '/' . $context['user']['language'] . '/quote.gif" alt="' . $txt[145] . '" />' : $txt[145]) . '</a>';
				}
				if ($topic['can_mark_notify'])
					$buttonArray[] = '<a href="' . $scripturl . '?action=notify;topic=' . $topic['id'] . '.' . $message['start'] . '">' . ($settings['use_image_buttons'] ? '<img src="' . $settings['images_url'] . '/' . $context['user']['language'] . '/notify_sm.gif" alt="' . $txt[131] . '" />' : $txt[131]) . '</a>';
						...
						<td>
							<table width="100%" cellpadding="4" cellspacing="1" border="0" class="bordercolor">
								<tr class="titlebg">
									...
									<td class="catbg" align="right">&nbsp;', implode($context['menu_separator'], $buttonArray), '</td>
								</tr>
							</table>
						</td>
					</tr>
