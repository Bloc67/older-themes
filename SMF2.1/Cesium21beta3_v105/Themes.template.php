...
function template_main()
{
	...
	foreach ($context['themes'] as $theme)
		echo '
						<div class="bwcell8"><div class="bwgutter_right"><div class="information overflow">
							<img class="floatright shadow" src="' , $theme['images_url'] , '/thumbnail.png" alt="" /><br>
							<input type="radio" name="options[theme_guests]" id="theme_guests" value="', $theme['id'], '"', $modSettings['theme_guests'] == $theme['id'] ? ' checked' : '', '>
								', $theme['name'], '
						</div></div></div>';
	...
	// Link to bloc's themes!
	echo '
		<div class="cat_bar">
			<h3 class="catbg">
				More of Bloc\'s Themes
			</h3>
		</div>
		<div class="windowbg2 noup">
			', $txt['blocthemes'], '
...
function template_set_settings()
{
		...
		if($settings['cesium_updates']->version > $settings['internal_revision'])
			echo '
					<div class="noticebox">
						...
						<div class="single_action floatright" style="margin: 1rem 0 0 1rem;"><a href="' , $settings['cesium_updates']->url , '">Visit</a></div>
						<b>',$settings['cesium_updates']->subject,'</b><br>' , $settings['cesium_updates']->body , '
					</div>';
	...
	<div id="admin_form_wrapper">
		<form action="', $scripturl, '?action=admin;area=theme;sa=list;th=', $context['theme_settings']['theme_id'], '" method="post" accept-charset="', $context['character_set'], '">
			...
			<img src="' , $context['theme_settings']['images_url'] , '/thumbnail.png" alt="" />';
	...
	foreach ($context['settings'] as $i => $setting)
	{
		...
		echo '
					<dt' , !empty($setting['container']) ? ' class="ces_container ' . $setting['container'] . '" ' . (empty($settings[$setting['container']]) ? 'style="display: none;"': '') : '' , '>
		...
		else
		{
			...
			if(!empty($setting['function']) && function_exists($setting['function']))
				echo '
				<span class="icon-chevron-down" style="cursor: pointer; display: inline-block; margin: 0.5rem 0 1rem 0rem;" onclick="fPop_toggle(\'#cesboard_'.$setting['id'].'_form\'); return false;"></span>', $txt['ces_showboards'], '<br>',$setting['function']($setting['id']);
...
function ces_boards($id)
{
	global $sourcedir, $boards,$settings;
	...
	echo '
	<div class="bwgrid" id="cesboard_'.$id.'_form" style="display: none;">';
	
	if(!empty($settings[$id]))
		$already = explode(",",$settings[$id]);
	else
		$already = array();

	// get the boards
	foreach($boards as $b => $bdata)
		echo '
		<div class="bwcell8">
			<input class="cboard'.$id.'" name="cesboard_' . $id . '_'.$b.'" id="cesboard_' . $id . '_'.$b.'" value="' . $b .'" type="checkbox"' , in_array($b,$already) ? ' checked' : '' , ' style="display: block; float: left; width: 10% !important; clear: left;margin-top: 5px !important;"> 
			<label for="cesboard_' . $id . '_'.$b.'">' , $bdata['name'], '</label>
			<hr>
		</div>';
	echo '
<script type=\'text/javascript\'>//<![CDATA[
	$(\'#cesboard_'.$id.'_form\').on(\'click\', \'input.cboard'.$id.'\', function() {
		var values = $.map($(\'input.cboard'.$id.':checked\'), function(el) {
			 return el.value;
		});
		$(\'#options_', $id, '\').val(values); 
	});
//]]> 
</script>
...
function ces_actions($id)
{
	global $sourcedir, $boards,$settings;

	echo '
	<div class="bwgrid" id="cesaction_'.$id.'_form" >';
	...
	// get the actions
	foreach(array('forum','search','calendar','admin','moderate','mlist') as $b)
		echo '
		<div class="bwcell8">
			<input class="caction'.$id.'" name="cesaction_' . $id . '_'.$b.'" id="cesaction_' . $id . '_'.$b.'" value="' . $b .'" type="checkbox"' , in_array($b,$already) ? ' checked' : '' , ' style="display: block; float: left; width: 10% !important; clear: left;margin-top: 5px !important;"> 
			<label for="cesaction_' . $id . '_'.$b.'">' , $b, '</label>
			<hr>
		</div>';
	echo '
<script type=\'text/javascript\'>//<![CDATA[
	$(\'#cesaction_'.$id.'_form\').on(\'click\', \'input.caction'.$id.'\', function() {
		var values = $.map($(\'input.caction'.$id.':checked\'), function(el) {
			 return el.value;
		});
		$(\'#options_', $id, '\').val(values); 
	});
...
function template_edit_browse()
{
	...
	foreach ($context['theme_files'] as $file)
	{
		...
		echo '
			<tr class="windowbg">
				<td data-label="', $txt['themeadmin_edit_filename'],'">';
		...
		echo '
				...
				<td class="righttext" data-label="', $txt['themeadmin_edit_modified'],'">', !empty($file['last_modified']) ? $file['last_modified'] : '', '</td>
				<td class="righttext" data-label="', $txt['themeadmin_edit_size'],'">', $file['size'], '</td>
			</tr>';
	}
