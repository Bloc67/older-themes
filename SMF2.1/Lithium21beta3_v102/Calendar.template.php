...
function template_show_upcoming_list($grid_name)
{
	...
	<div>
		<ul class="menu_nav dropmenu nolist">
			<li><a href="', $scripturl, '?action=calendar;viewlist;year=', $context['current_year'], ';month=', $context['current_month'], ';day=', $context['current_day'], '" class="button', $context['calendar_view'] == 'view_list' ? ' active' : '', '">', $txt['calendar_list'], '</a></li>
			<li><a href="', $scripturl, '?action=calendar;viewmonth;year=', $context['current_year'], ';month=', $context['current_month'], '" class="button', $context['calendar_view'] == 'view_month' ? ' active' : '', '">', $txt['calendar_month'], '</a></li>
			<li><a href="', $scripturl, '?action=calendar;viewweek;year=', $context['current_year'], ';month=', $context['current_month'], ';day=', $context['current_day'], '" class="button', $context['calendar_view'] == 'view_week' ? ' active' : '', '">', $txt['calendar_week'], '</a></li>
		</ul>
	</div>';
	...
	if (!empty($calendar_data['events']))
	{
		echo '
			<div>
				<div class="cat_bar">
					<h3 class="catbg">', str_replace(':', '', $txt['events']), '</h3>
				</div>
				<ul class="nolist">';
		...
		foreach ($calendar_data['events'] as $date => $date_events)
		{
			foreach ($date_events as $event)
			{
				echo '
					<li>
						<span class="floatleft clear_left" style="margin-top: 4px;"><b>', $event['link'], '</b>';
				...
				{
					echo '<time style="display: inline;" datetime="' . $event['start_iso_gmdate'] . '">', trim($event['start_date_local']), '</time>', ($event['start_date'] != $event['end_date']) ? ' &ndash; <time  style="display: inline;" datetime="' . $event['end_iso_gmdate'] . '">' . trim($event['end_date_local']) . '</time>' : '';
				}
				else
				{
					// Display event info relative to user's local timezone
					echo '<time style="display: inline;"  datetime="' . $event['start_iso_gmdate'] . '">', trim($event['start_date_local']), ', ', trim($event['start_time_local']), '</time> &ndash; <time  style="display: inline;" datetime="' . $event['end_iso_gmdate'] . '">';
					...
					{
						echo '</time> (<time style="display: inline;"  datetime="' . $event['start_iso_gmdate'] . '">';

						if ($event['start_date_orig'] != $event['start_date_local'] || $event['end_date_orig'] != $event['end_date_local'] || $event['start_date_orig'] != $event['end_date_orig'])
							echo trim($event['start_date_orig']), ', ';

						echo trim($event['start_time_orig']), '</time> &ndash; <time  style="display: inline;" datetime="' . $event['end_iso_gmdate'] . '">';
				...
				if (!empty($event['location']))
					echo '<span  style="display: inline;" class="location">', $event['location'],'</span>';
	...
	if (!empty($calendar_data['holidays']))
	{
		echo '
			<div><br>
				<div class="cat_bar">
					<h3 class="catbg">', str_replace(':', '', $txt['calendar_prompt']), '</h3>
				</div>
				<div class="windowbg">
...
function template_show_month_grid($grid_name, $is_mini = false)
{
				...
				if (empty($calendar_data['previous_calendar']['disabled']) && $calendar_data['show_next_prev'] && $is_mini === false)
				{
					echo '
						<span class="floatleft">
							<a href="', $calendar_data['previous_calendar']['href'], '"><span class="icon-arrow-left"></span></a>
						</span>
					';
				}
				...
				if (empty($calendar_data['next_calendar']['disabled']) && $calendar_data['show_next_prev'] && $is_mini === false)
				{
					echo '
						<span class="floatright">
							<a href="', $calendar_data['next_calendar']['href'], '"><span class="icon-arrow-right"></span></a>
						</span>
					';
				}
	...
	echo '<table class="table_grid_opt table_calendar">';
	...
	foreach ($calendar_data['weeks'] as $week)
	{
		...
		{
			echo '
				<td class="cal_week" style="width: 2%;">
					<a href="', $scripturl, '?action=calendar;viewweek;year=', $calendar_data['current_year'], ';month=', $calendar_data['current_month'], ';day=', $week['days'][0]['day'], '" title="', $txt['calendar_view_week'], '">
		...
		foreach ($week['days'] as $day)
		{
			...
			if (!empty($day['day']))
			{
				...
				$classes[] = !empty($calendar_data['size']) && $calendar_data['size'] == 'small' ? 'compact' : '';
				$show_small = !empty($calendar_data['size']) && $calendar_data['size'] == 'small' ? true : false;
				$classes[] = !empty($day['is_today']) ? 'cal_today' : '';
			...
			echo '<td class="w14p calheight ', implode(' ', $classes), '">';
			...
			if (!empty($day['day']))
			{
				...
				$title_prefix = !empty($day['is_first_of_month']) && $context['current_month'] == $calendar_data['current_month']  ? (!empty($calendar_data['short_month_titles']) ? $txt['months_short'][$calendar_data['current_month']] . ' ' : $txt['months_titles'][$calendar_data['current_month']] . ' ') : '';
				...
				if ($is_mini === false)
				{
					...
					if (!empty($day['holidays']))
						echo '<div class="holiday">', $txt['calendar_prompt'], ' <b>', implode(', ', $day['holidays']), '</b></div>';
						...
						foreach ($day['birthdays'] as $member)
							echo '
								<a href="', $scripturl, '?action=profile;u=', $member['id'], '">
									<span class="icon-color-birthday"></span>
									<span class="fix_rtl_names"><b>', $member['name'], '</b></span>
					...
					if (!empty($day['events']))
					{
						...
						subtemplate_calendar_items($day['events'], $show_small);
			...
			{
				echo '<a href="', $scripturl, '?action=calendar;year=', $context['calendar_grid_next']['current_year'], ';month=', $context['calendar_grid_next']['current_month'], '">', $current_month_started + 1 == $count ? (!empty($calendar_data['short_month_titles']) ? $txt['months_short'][$context['calendar_grid_next']['current_month']] . ' ' : $txt['months_titles'][$context['calendar_grid_next']['current_month']] . ' ') : '',  '</a>';
				$final_count++;
			}
...
function template_show_week_grid($grid_name)
{
	...
	foreach ($calendar_data['months'] as $month_data)
	{
			...
			echo '
				<div class="cat_bar">
					...
					if (empty($calendar_data['previous_calendar']['disabled']) && !empty($calendar_data['show_next_prev']))
					{
						echo '
							<span class="floatleft">
								<a href="', $calendar_data['previous_week']['href'], '"><span class="icon-arrow-left"></span></a>
							</span>
						';
					}
					...
					if (empty($calendar_data['next_calendar']['disabled']) && !empty($calendar_data['show_next_prev']))
					{
						echo '
							<span class="floatright">
								<a href="', $calendar_data['next_week']['href'], '"><span class="icon-arrow-right"></span></a>
							</span>';
					}
		...
		echo '
			<table class="table_grid_opt calendar_week">
				<tr class="top_text des">
					<th class="days w4" scope="col">', $txt['calendar_day'], '</th>
					...
					echo '
						<tr class="days_wrapper">
							...
							if (!empty($context['can_post']))
								echo '
										<a href="', $scripturl, '?action=calendar;sa=post;month=', $month_data['current_month'], ';year=', $month_data['current_year'], ';day=', $day['day'], ';', $context['session_var'], '=', $context['session_id'], '" class="mob"><span class="icon-write floatright" title="', $txt['calendar_post_event'], '"></span></a>';
							// Should the day number be a link?
							if (!empty($modSettings['cal_daysaslink']) && $context['can_post'])
								echo '<a href="', $scripturl, '?action=calendar;sa=post;month=', $month_data['current_month'], ';year=', $month_data['current_year'], ';day=', $day['day'], ';', $context['session_var'], '=', $context['session_id'], '"><span class="cal_day">', $day['day'], '</span>', $txt['days'][$day['day_of_week']], '</a>';
							else
								echo '<span class="cal_day">',$day['day'], '</span>' , $txt['days'][$day['day_of_week']];
							...
							if (!empty($day['events']))
							{
								...
								if (!empty($context['can_post']))
								{
									...
									<div class="week_add_event">
										<a href="', $scripturl, '?action=calendar;sa=post;month=', $month_data['current_month'], ';year=', $month_data['current_year'], ';day=', $day['day'], ';', $context['session_var'], '=', $context['session_id'], '"><span class="icon-write" title="', $txt['calendar_post_event'], '"></span></a>
									</div>
									<br class="clear">';
								}
							}
							...
							if (!empty($day['birthdays']))
							{
								foreach ($day['birthdays'] as $member)
								{
									echo '
										<a href="', $scripturl, '?action=profile;u=', $member['id'], '">
											<span class="icon-color-birthday"></span> ', $member['name'], '
										</a>
										', isset($member['age']) ? ' (' . $member['age'] . ')' : '', '
										', $member['is_last'] ? '' : '<br>';
								}
...
function template_calendar_top($calendar_data)
{
	...
	{
		echo '
			<form action="', $scripturl, '?action=calendar;viewlist" id="calendar_range" method="post" accept-charset="', $context['character_set'], '" class="floatleft">
			...
			<div class="roundframe noup">
				<fieldset id="event_main">
					...
					<div class="event_options_left" id="event_title">
						<div>
							<input type="text" id="evtitle" name="evtitle" maxlength="255" style="width: 99%;" value="', $context['event']['title'], '" tabindex="', $context['tabindex']++, '" class="input_text">
						</div>
					</div>';
...
function template_bcd()
{
	...
	$alt = false;
	echo '
		<table class="table_grid_opt" style="margin: 0 auto 0 auto; border: 1px solid #ccc;">
			<tr>
