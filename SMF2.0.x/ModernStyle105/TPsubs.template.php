...
function tpo_whos($buddy_only = false)
{
	...
	if(isset($whos['users_online']) && count($whos['users_online']) > 0)
	{
		...
		foreach($avy as $a => $av)
			echo '
		<a class="avatar_single" title="'.$names[$a].'" href="' . $scripturl . '?action=profile;u='.$a.'" style="margin: 0.5em 0; display: block;">' . $av . '</a>';
	}
}
	...
	else
	{
		...
		foreach($what as $wi => $w)
		{
			...
			<li style="border: none; margin: 0;padding: 0;">
				<h4 class="titlebg"><a href="'.$w['href'].'">' . $w['short_subject'].'</a></h4>
				<span class="bwgrid" style="display: block;">
					<a href="' . $scripturl. '?action=profile;u=' . $w['poster']['id'] . '" class="bwcell3 avatar_single" style="display: block;">' , empty($w['poster']['avatar']) ? '<img src="' . $settings['tp_images_url'] . '/TPguest.png" alt="" />' : $w['poster']['avatar'] , '</a>
					<span class="bwcell13" style="display: block;">
						<span class="greytext middletext">', $txt['by'], ' <b>', $w['poster']['link'],'</b> ';
			if(!$w['new'])
				echo ' <a href="'.$w['href'].'"><img src="'. $settings['images_url'].'/'.$context['user']['language'].'/new.gif" alt="new" /></a> ';
			...
			echo '	</span>
				...
				<span class="smalltext">'.$w['time'].'</span><hr>
			</li>';
			$coun++;
		}
		echo '
...
function TPortal_userbox()
{
	...
	if ($context['user']['is_logged'])
	{
		...
		if (!empty($context['user']['avatar']) &&  isset($context['TPortal']['userbox']['avatar']))
			echo '
				<span class="ravatar" style="margin: 0 1rem 2rem 0; vertical-align: 100%; background-image: url(', $context['user']['avatar']['href'], ');"></span>';
		echo '
		<h3 class="titlebg">', $context['user']['name'], '</h3>
         ...
         echo '
			...
			<input class="button_submit" type="button" value="'.$txt['tp-changetheme'].'" onclick="jumpit()" /><br><br>
 			...
 			<div style="text-align: center; ">
				<img src="'.$settings['images_url'].'/thumbnail.gif" alt="" id="chosen" name="chosen" style="max-width: 100%; width: 100%;" />
			</div>
		</form>
...
function TPblock($block, $theme, $side, $double=false)
{
	...
	{
		echo	'
	<div class="tp"><h3 class="titlebg nomarginb"><span class="floatright">';
		...
		if ($theme || $block['frame'] == 'title')
		{
			...
			if($block['visible'] == '' || $block['visible'] == '1')
				echo '<a href="javascript: void(0); return false" onclick="ms_toggle(\''.$block['id'].'\'); return false">&nbsp;<span id="blockcollapse'.$block['id'].'" class="icon-toggle-' , !in_array($block['id'],$context['TPortal']['upshrinkblocks'])  ? 'off' : 'on' , '" title="'.$txt['block-upshrink_description'].'"></span></a>';

			// can you edit the block?
			if($block['can_edit'] && !$context['TPortal']['blocks_edithide'])
				echo '<a href="',$scripturl,'?action=tpmod;sa=editblock'.$block['id'].';' . $context['session_var'] . '=' . $context['session_id'].'">&nbsp;<span class="icon-edit" title="'.$txt['edit_description'].'"></span></a>';
			elseif($block['can_manage'] && !$context['TPortal']['blocks_edithide'])
				echo '<a href="',$scripturl,'?action=tpadmin;blockedit='.$block['id'].';' . $context['session_var'] . '=' . $context['session_id'].'">&nbsp;<span class="icon-edit" title="'.$txt['edit_description'].'"></span></a>';

			echo '</span>' . $block['title'];
		...
		<div>';
				if($block['visible'] == '' || $block['visible'] == '1')
					echo '<a href="javascript: void(0); return false" onclick="ms_toggle(\''.$block['id'].'\'); return false" class="floatright"><span id="blockcollapse'.$block['id'].'" class="icon-toggle-' , !in_array($block['id'],$context['TPortal']['upshrinkblocks'])  ? 'off' : 'on' , '" title="'.$txt['block-upshrink_description'].'"></span></a>';
				echo '&nbsp;
		</div>';
			}
		}
		echo '
		<div ', in_array($block['id'],$context['TPortal']['upshrinkblocks']) ? ' style="display: none;"' : ''  , ' id="block'.$block['id'].'">';
...
{
	template_button_strip($button_strip, '', array() ,true);
}
