...
function template_set_settings()
{
	...
	foreach ($context['settings'] as $setting)
	{
		...
		elseif ($setting['type'] == 'multicheckbox')
		{
			...
			echo '
						...
						<dd>';
			$selected = !empty($setting['value']) ? explode(",",$setting['value']) : array();
			foreach($setting['stub']() as $id => $name)
			{
				echo '
							<div class="bwcell8"><input class="mul_', $setting['id'] ,'" onclick="find_class_js(\'mul_', $setting['id'] ,'\', \'', $setting['id'], '\')" type="checkbox" id="mul_', $id , '_' , $setting['id'], '"', in_array($id,$selected) ? ' checked="checked"' : '', ' value="',$id,'" class="input_check" />&nbsp; ', $name,'</div>';
			}
			echo '		<br class="clear">
							<input type="hidden" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '" value="', $setting['value'], '"', $setting['type'] == 'number' ? ' size="5"' : (empty($setting['size']) ? ' size="40"' : ' size="' . $setting['size'] . '"'), ' class="input_text" />
						</dd>';
		}
		// A list with options?
		elseif ($setting['type'] == 'list')
		...
		else
		{
			...
			echo '
						...
						<dd>
							<input type="text" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '" value="', $setting['value'], '"', $setting['type'] == 'number' ? ' size="5"' : (empty($setting['size']) ? ' size="40"' : ' size="' . $setting['size'] . '"'), ' class="input_text"', $setting['type'] == 'number' ? ' style="width: 5em;"' : '', ' />
						</dd>';
		}
	}
	...
	echo '
		...
		<script>
			function find_class_js(what, target)
			{
				var checkBoxez = document.getElementsByClassName(what);
				var m;
				var vals = new Array;

				for (m = 0; m < checkBoxez.length; m++) {
					if(checkBoxez[m].checked)
					{
						vals.push(checkBoxez[m].value);
					}
				}
				document.getElementById(target).value = vals.join(\',\');
			}
		</script>
	<br class="clear" />';
		...
		echo '
			...
			<div class="', $theme['selected'] ? 'windowbg' : 'windowbg2', '">
				...
				<div class="flow_hidden content">
					<div class="floatright"><a href="', $scripturl, '?action=theme;sa=pick;u=', $context['current_member'], ';theme=', $theme['id'], ';', $context['session_var'], '=', $context['session_id'], '" id="theme_thumb_preview_', $theme['id'], '" title="', $txt['theme_preview'], '"><img src="', $theme['thumbnail_href'], '" id="theme_thumb_', $theme['id'], '" alt="" class="shadow" /></a></div>
