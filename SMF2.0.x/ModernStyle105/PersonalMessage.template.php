...
<form class="flow_hidden" action="', $scripturl, '?action=pm;sa=pmactions;', $context['display_mode'] == 2 ? 'conversation;' : '', 'f=', $context['folder'], ';start=', $context['start'], $context['current_label_id'] != -1 ? ';l=' . $context['current_label_id'] : '', '" method="post" accept-charset="', $context['character_set'], '" name="pmFolder">';
	...
	if ($context['get_pmessage']('message', true))
	{
		...
		while ($message = $context['get_pmessage']('message'))
		{
			if (empty($context['display_mode']))
				echo '
					<input type="checkbox" name="pms[]" id="deletedisplay', $message['id'], '" value="', $message['id'], '" onclick="document.getElementById(\'deletelisting', $message['id'], '\').checked = this.checked;" class="input_check floatright" />';

			echo '
		<div class="bwgrid">
			<div class="bwcell2"><div style="padding-top: 1em;">';
			...
			<div class="bwcell13">
				<div class="catbg">
					<h3 style="margin: 0;">', $message['member']['link'],'</h3>
				</div>
				<span class="greytext smalltext">', $txt['sent_to'], ' ';
	...
	<div class="pagesection">
		<div class="floatleft">', convertpages($context['page_index']), '</div>
		...
		elseif ($context['display_mode'] == 2 && isset($conversation_buttons))
			echo '
	<div class="pagesection">', template_button_strip($conversation_buttons, 'right'), '</div>';
...
function template_subject_list()
{
	...
	while ($message = $context['get_pmessage']('subject'))
	{
		...
		echo '
			...
			// ]]></script>
				', $message['is_replied_to'] ? '<span class="icon-check" title="' . $txt['pm_replied'] . '"></span>' : '<span class="icon-home" title="' . $txt['pm_read'] . '"></span>', '</td>
			<td><span class="smalltext">', $message['time'], '</span></td>
			<td>', ($context['display_mode'] != 0 && $context['current_pm'] == $message['id'] ? '<span class="icon-right"></span>' : ''), '<a href="', ($context['display_mode'] == 0 || $context['current_pm'] == $message['id'] ? '' : ($scripturl . '?action=pm;pmid=' . $message['id'] . ';kstart;f=' . $context['folder'] . ';start=' . $context['start'] . ';sort=' . $context['sort_by'] . ($context['sort_direction'] == 'up' ? ';' : ';desc') . ($context['current_label_id'] != -1 ? ';l=' . $context['current_label_id'] : ''))), '#msg', $message['id'], '">', $message['subject'], '</a>', $message['is_unread'] ? '&nbsp;<img src="' . $settings['lang_images_url'] . '/new.gif" alt="' . $txt['new'] . '" />' : '', '</td>
