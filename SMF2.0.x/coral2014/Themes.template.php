	...
	foreach ($context['settings'] as $setting)
	{
		...
		elseif ($setting['type'] == 'list')
		{
			...
			echo '
						...
						<dd>
							<select name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '" style="width: 90%;">';
		...
		elseif ($setting['type'] == 'radio')
		{
			...
			foreach ($setting['vals'] as $value => $label)
				echo '
						<span style="display: block; float: left; margin: 5px; ">
							<input type="radio" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '" value="', $value, '"', $value == $setting['value'] ? ' checked' : '', '>
							' , $label , '<br><img src="', $settings['images_url'], '/' , $label, '" />
						</span>';
		...
		elseif ($setting['type']=='social')
		{
			echo '
						<dt>
							<label for="', $setting['id'], '">', $setting['label'], '</label><span class="socialbuttons" id="', $setting['socialclass'], '"></span>
						</dt>
						<dd>
							<input type="text" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '" value="', $setting['value'], '"', $setting['type'] == 'number' ? ' size="5"' : (empty($setting['size']) ? ' size="40"' : ' size="' . $setting['size'] . '"'), ' class="input_text" style="width: 90%;" />';
			if (isset($setting['description']))
				echo '<br />
		...
		elseif ($setting['type']=='textarea')
		{
			...
			echo '
						...
						<dd>
							<textarea style="height: 100px; width: 90%;" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '">', $setting['value'], '</textarea>
						</dd>';
		}
