...
{
	global $context, $settings, $options, $scripturl, $txt, $smcFunc, $modSettings;
	...
	echo '
			...
			--></script>
	<div id="mythemesettings"><br>
		<div class="title_bar"><h3 class="titlebg">'.$txt['bloc_custom'].'</h3></div>
		<div class="windowbg2"><div class="content"><dl class="settings">';		
			
	foreach ($context['settings'] as $setting)
	{
		// skip any regular ones
		if(!isset($setting['customsetting']) || (isset($setting['customsetting']) && isset($setting['widget'])))
			continue;
		...
		if ($setting['type'] == 'checkbox')
		{
			...
			if (isset($setting['description']))
				echo '
							<div class="theme_descript"><div class="innerf">', $setting['description'], '</div></div>';
			...
			echo '
						...
						<dd>
							...
							' , !empty($setting['customcode']) ? $setting['customcode'] : '' , '
						</dd>';
		}
		elseif ($setting['type'] == 'radioimage')
		{
			echo '
		</dl>
			<label for="', $setting['id'], '">', $setting['label'], '</label><hr>
			<div style="overflow: hidden;">';
			foreach ($setting['choices'] as $value => $label)
			{
				echo '<div style="overflow: hidden; float: left; text-align: center; margin: 5px;">
				<input type="radio" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '" value="', $value, '"', $value == $setting['value'] ? ' checked="checked"' : '', '>
					<br><img style="margin-top: 5px;" src="' . $settings['images_url']. '/' . $label . '" alt="" />
				</div>';
			}
			echo '</div><hr>
			<dl class="settings">';
		...
		{
			echo '<dt><label for="', $setting['id'], '">', $setting['label'], '</label>:';
			...
			if (isset($setting['showboards']))
			{
				...
				echo '
					<div class="information" ><div style="padding: 0px; height: 90px; overflow: auto;">'; 
					
				foreach($boards as $b)
					echo '<div style="width: 48%; float: left;">'. $b['name'].' =><b> '.$b['id'].'</b></div>';
		...
		else
		{
			...
			if (isset($setting['showboards']))
			{
				...
				echo '
					<div class="information" ><div style="padding: 1em; height: 60px; overflow: auto;">'; 
					
				foreach($boards as $b)
					echo $b['name'].' =><b> '.$b['id'].'</b><br>';
	...
	foreach ($context['settings'] as $setting)
	{
		// skip any custom ones
		if(isset($setting['customsetting']) || isset($setting['widget']))
			continue;
		...
		elseif ($setting['type'] == 'radioimage')
		{
			echo '<dt><label for="', $setting['id'], '">', $setting['label'], '</label></dt><dd>
			<div style="overflow: hidden;">';
			foreach ($setting['choices'] as $value => $label)
			{
				echo '<div style="overflow: hidden; float: left; height: 160px; width: 300px;margin: 5px;">
				<input type="radio" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '" value="', $value, '"', $value == $setting['value'] ? ' checked="checked"' : '', '>
					<span', $value == $setting['value'] ? ' style="font-weight: bold;"' : '', '> ' , $label, '</span><br /><img style="margin-top: 5px;" src="' , $settings['images_url'] , '/theme/bg/darken' , $value , '.jpg" alt="c_' , $value , '" />
						</div>';
			}
			echo '</div></dd>';
		}
