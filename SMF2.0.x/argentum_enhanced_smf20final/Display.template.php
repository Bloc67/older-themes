...
function template_main()
{
	...
	{
		if($settings['display_type']==1)
		{
			if(!empty($settings['display_blogboards']) && in_array($context['current_board'], explode(",",$settings['display_blogboards'])))
			{
				loadtemplate('Display2');
				template_blog();
				return;
			}
			elseif(empty($settings['display_blogboards']))
			{
				loadtemplate('Display2');
				template_blog();
				return;
	...
	while ($message = $context['get_message']())
	{
		...
		if (!$message['member']['is_guest'])
		{
			...
			<div class="smalltext" style="padding: 0 0 0 15px;" id="buddybox'.$message['id'].'">';
	...
	if ($context['can_reply'] && !empty($options['display_quick_reply']))
	{
		echo '
			<a id="quickreply"></a>
			<div id="quickreplybox" class="sreply" style="' , ((!empty($options['quickhide']) && !empty($settings['staticreplybox'])) || empty($settings['staticreplybox'])) ? 'position: relative;bottom: 0;width: auto;' : '' ,  !empty($options['quicksmall']) && !empty($settings['staticreplybox']) ? 'height: 30px;' : '' , ' ">
				<h3 class="windowbg3" style="padding: 0.5em; border-bottom: solid 1px #aaa;">';
		if(!empty($settings['staticreplybox']))
			echo '<span style="float: right;">
		<img title="Normal position" id="quickreplyhide" alt="" style="cursor: pointer;" src="'.$settings['images_url'].'/theme/quickhide.png" />  
		<img title="Fixed position"  id="quickreplyshow" alt="" style="cursor: pointer;"  src="'.$settings['images_url'].'/theme/quickshow.png" />  
		<img title="Upshrinked"  id="quickreplydark" alt="" style="cursor: pointer;"  src="'.$settings['images_url'].'/theme/quickdark.png" />  
		<img title="Full height"  id="quickreplylight" alt="" style="cursor: pointer;"  src="'.$settings['images_url'].'/theme/quicklight.png" />  
					</span>';
		echo '
						', $txt['quick_reply'], '
				</h3>
	...
	<script type="text/javascript" >';
		...
		window.addEvent(\'domready\', function() {
			...
			$(\'quickreplydark\').addEvent(\'click\', function(e){
				smf_setThemeOption(\'quicksmall\', \'1\', '.$settings['theme_id'].', \''.$context['session_id'].'\' , \''.$context['session_var'].'\', \'\');
				e.stop();
				qr.tween(\'height\',\'30px\');
			});
			$(\'quickreplylight\').addEvent(\'click\', function(e){
				smf_setThemeOption(\'quicksmall\', \'0\', '.$settings['theme_id'].', \''.$context['session_id'].'\' , \''.$context['session_var'].'\', \'\');
				e.stop();
				qr.tween(\'height\',\'225px\');
			});
		});	
	</script>
