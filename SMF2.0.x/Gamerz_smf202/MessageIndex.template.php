...
function template_main()
{
	...
	$is_blog = $is_gallery = false;
	// blogs?
	if(!empty($settings['blogboards']) && in_array($context['current_board'],explode(",",$settings['blogboards'])))
		$is_blog = true;
	// gallery?
	if(!empty($settings['galleryboards']) && in_array($context['current_board'],explode(",",$settings['galleryboards'])))
	{
		$is_blog = false;
		$is_gallery = true;
	}
	...
	{
		foreach($context['topics'] as $b)
			$membs[$b['first_post']['member']['id']] = $b['first_post']['member']['id'];
	}	
	if(count($membs)>0)
	{
		...
		foreach ($context['boards'] as $board)
		{
			echo '
				<tr id="board_', $board['id'], '" class="windowbg2">
					<td class="icon windowbg"', !empty($board['children']) ? ' rowspan="2"' : '', ' style="background: #000;">
			...
			if(empty($settings['no_rss']))
				echo '
				<a href="' , $scripturl , '?action=.xml;board=' .$board['id']. ';type=rss" class="floatright" style="margin: 0 0 0 5px;"><img src="' , $settings['images_url'] , '/theme/rss.png" alt="RSS" /></a>';
			
			if(empty($settings['no_avvy']))
				echo '
				<a href="' , $scripturl , '?action=profile;u=' .$board['last_post']['member']['id']. ';type=rss" class="floatleft" style="margin: 5px 8px 1em 0px;">
	...
	<div class="tborder topic_table" id="messageindex">
		...
		if (!empty($context['topics']))
		{
			echo '
					...
					<th scope="col" width="14%" class="statsheader"><a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=replies', $context['sort_by'] == 'replies' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['replies'], $context['sort_by'] == 'replies' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a> / <a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=views', $context['sort_by'] == 'views' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['views'], $context['sort_by'] == 'views' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a></th>';
			// Show a "select all" box for quick moderation?
			if (empty($context['can_quick_mod']))
				echo '
					<th scope="col" class="lefttext last_th modbar" width="22%"><a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=last_post', $context['sort_by'] == 'last_post' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['last_post'], $context['sort_by'] == 'last_post' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a></th>';
			else
				echo '
					<th scope="col" class="lefttext modbar" width="22%"><a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=last_post', $context['sort_by'] == 'last_post' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['last_post'], $context['sort_by'] == 'last_post' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a></th>';

			// Show a "select all" box for quick moderation?
			if (!empty($context['can_quick_mod']) && $options['display_quick_mod'] == 1)
				echo '
					<th scope="col" class="last_th modbar" width="24"><input type="checkbox" onclick="invertAll(this, this.form, \'topics[]\');" class="input_check" /></th>';

			// If it's on in "image" mode, don't show anything but the column.
			elseif (!empty($context['can_quick_mod']))
				echo '
					<th class="last_th modbar" width="4%">&nbsp;</th>';
		}
		...
		foreach ($context['topics'] as $topic)
		{
			...
			echo '
				<tr>
					<td class="icon1 ', $color_class, '" style="text-align: center;">
				<a href="' , $scripturl , '?action=profile;u=' .$topic['first_post']['member']['id']. ';type=rss"  style="margin: 5px 8px 1em 0px;">
					<img alt=""  class="avyframe" style="width: 25px; float: none;" src="' . (!empty($avvy[$topic['first_post']['member']['id']]) ? $avvy[$topic['first_post']['member']['id']] : $settings['images_url'].'/theme/guest.png') . '" />
				</a>
					</td>
