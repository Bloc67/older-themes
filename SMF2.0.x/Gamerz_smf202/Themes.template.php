...
function template_set_settings()
{
	...
	$texts = array(
		...
		'Set blog boards.'.(!empty($settings['no_blog']) ? ' <span class="error">[INACTIVE]</span>' : '') ,
		'Set gallery boards.'.(!empty($settings['no_gallery']) ? ' <span class="error">[INACTIVE]</span>' : ''),
		'Edit the About page.'.(!empty($settings['no_about']) ? ' <span class="error">[INACTIVE]</span>' : ''),
		);
	foreach(array('menu','blog','gallery','about') as $count => $ico)
		echo '<div class="floatleft" style="text-align: center; width: 30%; padding: 1%;">
	<a href="' , $scripturl , '?action=mysettings;sa=' . $ico . '"><img style="margin: 1em;" src="' . $settings['images_url'] . '/settings/' . $ico . '.png" alt="" /></a>
				<h3>'. $headers[$count] . '</h3><em class="smalltext">' . $texts[$count] .' </em>
				</div>';
	...
	echo '
			...
			<div class="windowbg2">
				<span class="topslice"><span></span></span>
				<div class="content" style="margin: 0; padding: 0 1em;">
					<dl class="settings">
						<dt style="width: 100%; padding: 0;margin: 0;">
							<label for="theme_name" style="padding: 0;">', $txt['actual_theme_name'], '</label>
						</dt>
						<dd style="width: 100%; padding: 0;">
							<input type="text" id="theme_name" name="options[name]" value="', $context['theme_settings']['name'], '" size="32" class="input_text" style="width: 98%;" />
						</dd>
						<dt style="width: 100%; padding: 0;margin: 0;">
							<label for="theme_url">', $txt['actual_theme_url'], '</label>
						</dt>
						<dd style="width: 100%; padding: 0;">
							<input type="text" id="theme_url" name="options[theme_url]" value="', $context['theme_settings']['actual_theme_url'], '" size="50" style="max-width: 100%; width: 98%;" class="input_text" />
						</dd>
						<dt style="width: 100%; padding: 0;margin: 0;">
							<label for="images_url">', $txt['actual_images_url'], '</label>
						</dt>
						<dd style="width: 100%; padding: 0;">
							<input type="text" id="images_url" name="options[images_url]" value="', $context['theme_settings']['actual_images_url'], '" size="50" style="max-width: 100%; width: 98%;" class="input_text" />
						</dd>
						<dt style="width: 100%; padding: 0;margin: 0;">
							<label for="theme_dir">', $txt['actual_theme_dir'], '</label>
						</dt>
						<dd style="width: 100%; padding: 0;">
							<input type="text" id="theme_dir" name="options[theme_dir]" value="', $context['theme_settings']['actual_theme_dir'], '" size="50" style="max-width: 100%; width: 98%;" class="input_text" />
						</dd>
					</dl>
				</div>
				<span class="botslice"><span></span></span>
	...
	foreach ($context['settings'] as $setting)
	{
		...
		elseif ($setting['type'] == 'textarea')
		{
			...
			if(!empty($setting['function']) && function_exists($setting['function']))
				$listing = call_user_func($setting['function']);

			echo '<dt><label for="', $setting['id'], '">', $setting['label'], '</label><div style="padding: 5px;">' , $listing , '</div></dt><dd>
					
					<textarea style="width: 98%;' , !empty($setting['myheight']) ? 'height: '.$setting['myheight'].';' : '' , '" rows="4" cols="40" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '">', (empty($setting['value']) && isset($setting['empty'])) ? $setting['empty'] : $setting['value'], '</textarea>
					</dd>';
		}
		// A list with options?
		elseif ($setting['type'] == 'list')
...
function show_menubuttons()
{
	global $context;

	$listing = '- <em>Available items:</em> - <br />';
	foreach($context['menu_buttons'] as $menu => $data)
		$listing .= '<b>'.$menu.'</b> ['.$data['title'].'], ';
