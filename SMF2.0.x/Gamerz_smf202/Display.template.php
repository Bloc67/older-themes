...
function template_main()
{
	...
	{
		echo '
				<p id="whoisviewing" class="smalltext windowbg3">';
	...
	while ($message = $context['get_message']())
	{
		...
		$avy = $blogger = '';
		if($is_blog || $is_gallery)
		{
			if (!empty($settings['show_user_images']) && empty($options['show_no_avatars']) && !empty($message['member']['avatar']['image']))
				$avy =  '<a class="blogavy' . (!$first ? ' cavy' : '')  . '" href="'. $scripturl. '?action=profile;u='. $message['member']['id']. '">'. $message['member']['avatar']['image']. '</a>';
			
			$blogger = '<span class="blogger">' . ($is_gallery ? $txt['uploaded_by'] : $txt['written_by']) . $message['member']['link'] . '&nbsp;' . $txt['on'] . ' ' . $message['time'] . '</span>';
		}
		...
		echo '
				<div class="', $message['approved'] ? ($message['alternate'] == 0 ? 'windowbg' : 'windowbg2') : 'approvebg', '"', $first ? ' id="firstblog">' : '>' , '
					<span class="topslice"><span></span></span>
		...
		echo '
						...
						<div class="postarea"' , $is_gallery ? ' style="padding-top: 0;"' : '' , '>
							<div class="flow_hidden"' , $is_gallery ? ' style="margin-top: 0;"' : '' , '>';

// for the gallery....
		if (!empty($message['attachment']) && $is_gallery && $first)
		{
			echo '
								<div style="text-align: center; margin: 0px 0 2em 0; padding: 0; ">';
			...
			foreach ($message['attachment'] as $attachment)
			{
				...
				if ($attachment['is_image'] && $is_gallery)
				{
					echo '
							<div style="overflow: auto; margin: 3px 0;"><img src="' . $attachment['href'] . ';image" alt="" class="imgstyle2"  /></div>';
				}
		...
		echo '
								<div class="inner" id="msg_', $message['id'], '"', $is_gallery ? ' style="border: none;"' : '', '>', $message['body'], '</div>
							</div>';
		...
		{
			echo '
							<div id="msg_', $message['id'], '_footer" class="' , $is_gallery ? 'information' : 'attachments smalltext' , '">
			...
			foreach ($message['attachment'] as $attachment)
			{
				...
				if ($attachment['is_image'] && (!$is_gallery || !$first))
				{
