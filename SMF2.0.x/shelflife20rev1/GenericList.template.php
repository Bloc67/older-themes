...
function template_show_list($list_id = null)
{
	...
	{
		echo '
			<div class="themepadding flow_auto">';
	...
	echo '
			<table class="table_grid" cellspacing="0" style="width: ', !empty($cur_list['width']) ? $cur_list['width'] : '100%', '">';
	...
	elseif (!empty($cur_list['rows']))
	{
		...
		foreach ($cur_list['rows'] as $id => $row)
		{
			...
			{
				// exchange those pesky new containers...
				$newdata = fReplaceNew($row_data['value'],'<img src="' . $settings['lang_images_url'] . '/new.gif" alt="' . $txt['new'] . '" />','<span class="onoff_on">&nbsp;</span>');

				echo '
					<td', empty($row_data['class']) ? '' : ' class="' . $row_data['class'] . '"', empty($row_data['style']) ? '' : ' style="' . $row_data['style'] . '"', '>', $newdata, '</td>';
			}
			echo '
				</tr>';
	...
	{
		echo '
			<div class="flow_auto themepadding">';
...
function fReplaceNew($source,$target, $newstring)
{
	$return = str_replace($target, $newstring,$source);
	return $return;
}
