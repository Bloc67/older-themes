	...
	foreach ($context['settings'] as $setting)
	{
		...
		if (empty($setting))
		{
			...
			elseif ($setting['type'] == 'cssbackground')
			{
				...
				// get all boards
				if(isset($setting['att']))
					$settings['tp_cssbg'] = my_readfolder($settings['theme_dir'].'/images/theme',$settings['theme_url'].'/images/theme', '.'.$setting['att'], $setting['prefix']);
				else
				...
				foreach ($settings['tp_cssbg'] as $value => $path)
					echo '
								<label for="', $setting['id'].'_'.$value.'"' , isset($setting['att']) ? ' style="width: 25%; float: left;"' : '' , '><input style="float: left; margin-top: 1rem;" type="radio" name="options[', $setting['id'],']" id="', $setting['id'],'_' , $value , '" value="', $path, '"', $setting['value'] == $path ? ' checked' : '', '>
									<span style="', $setting['value'] == $path ? 'opacity: 0.5;' : '', 'margin: 0 0 2px 30px; height: 50px; display: block; background-image: url(' , $path , '); background-repeat: no-repeat; ' , isset($setting['att']) ? ' background-size: contain;' : '' , '"></span>
								</label>';
...
function my_readfolder($path, $url, $ext, $pre = '')
{
	$return = array();
	$dir = scandir($path);
	foreach($dir as $m => $file)
	{
