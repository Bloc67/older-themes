		...
		foreach ($context['topics'] as $topic)
		{
			...
			echo '
					...
					echo '			<div ', (!empty($topic['quick_mod']['modify']) ? 'id="topic_' . $topic['first_post']['id'] . '" onmouseout="mouse_on_div = 0;" onmouseover="mouse_on_div = 1;" ondblclick="modify_topic(\'' . $topic['id'] . '\', \'' . $topic['first_post']['id'] . '\', \'' . $context['session_id'] . '\', \'' . $context['session_var'] . '\');"' : ''), '>
									<strong><span id="msg_' . $topic['first_post']['id'] . '"><span class="mysubject"><a href="', $topic['first_post']['href'], '"  class="', $color_class, '">' , $topic['subject'] ,'</a></span>' . (!$context['can_approve_posts'] && !$topic['approved'] ? '&nbsp;<em>(' . $txt['awaiting_approval'] . ')</em>' : ''), '</span></strong>';
			...
			if(!empty($settings['avatarboards']))
				echo '
						<a href="', $topic['last_post']['href'], '"><img src="' . (!empty($avvy[$topic['last_post']['member']['id']]) ? $avvy[$topic['last_post']['member']['id']] : $settings['images_url'].'/none.png') . '" style="margin: 5px;float: right; max-height: 25px;" alt="" /></a>';
			else
