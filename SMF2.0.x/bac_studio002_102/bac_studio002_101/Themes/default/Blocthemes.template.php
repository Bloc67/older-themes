...
function show_btsettings($opts)
{
	...
	foreach ($opts as $setting)
	{
		...
		elseif ($setting['type'] == 'list')
		{
			...
			foreach ($setting['options'] as $value => $label)
				echo '
							<option value="', $value, '"', isset($setting['value']) && $value == $setting['value'] ? ' selected="selected"' : '', '>', $label, '</option>';
		...
		elseif ($setting['type'] == 'radio')
		{
			...
			foreach ($setting['options'] as $value => $label)
				echo '
							<input type="radio" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '" value="', $value, '"', isset($setting['value']) && $value == $setting['value'] ? ' checked="checked"' : '', '>', $label, ' <br>';
		...
		elseif ($setting['type'] == 'colorselect')
		{
			...
			echo '
						...
						<dd>
							...
							<div id="show_' . $setting['id'] . '" style="background: ' , isset($setting['value']) ? $setting['value'] : '' , '; display: inline-block; width: 200px; height: 60px;"></div>
   <script type="text/javascript">
      ColorPicker(
        ...
        function(hex, hsv, rgb) {
          ...
          document.getElementById(\'show_' . $setting['id'] . '\').style.backgroundColor = hex;   
							...
							<br><input type="text" name="options[', $setting['id'], ']" id="', $setting['id'], '" value="' , isset($setting['value']) ? $setting['value'] : '' , '" class="input_text" />
						</dd>';
		}
	...
	echo '
			...
			<input type="hidden" name="bt_theme_id" value="', $settings['theme_id'], '" />
		</form>
	</div>
	<br class="clear" />';
}
