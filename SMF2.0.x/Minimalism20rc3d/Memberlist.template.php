...
function template_main()
{
	...
	foreach ($context['columns'] as $column)
	{
		...
		// This is just some column... show the link and be done with it.
			...
			echo '
					<th scope="col" class="smalltex"', isset($column['width']) ? ' width="' . $column['width'] . '"' : '', isset($column['colspan']) ? ' colspan="' . $column['colspan'] . '"' : '', '>
						', $column['link'], '</th>';
	}
	echo '
				</tr>
		...
		foreach ($context['members'] as $member)
		{
			echo '
				<tr ', empty($member['sort_letter']) ? '' : ' id="letter' . $member['sort_letter'] . '"', '>
					<td class="windowbg2">
						<div class="avatar16h3">' , !empty($avy[$member['id']]) ? $avy[$member['id']] : '<img src="'.$settings['images_url'].'/guest.jpg" alt="*" />' , '</div>
