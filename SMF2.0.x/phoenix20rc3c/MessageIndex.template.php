...
{
	global $context, $smcFunc, $db_prefix, $settings, $options, $scripturl, $modSettings, $txt;
...
<div class="tborder" id="childboards">
	...
	<div class="table_frame">
		...
		foreach ($context['boards'] as $board)
		{
			...
			{
				echo '
				<td class="windowbg" valign="top" width="2%" style="padding: 2px;"><img style="height: ' , !empty($settings['avatarboards_height']) ? $settings['avatarboards_height'] : '60' , 'px;  width: ' , !empty($settings['avatarboards_width']) ? $settings['avatarboards_width'] : '60' , 'px;" src="' . (!empty($avvy[$board['last_post']['member']['id']]) ? $avvy[$board['last_post']['member']['id']] : $settings['images_url'].'/none.png') . '" /></td>';
			}
	...
	<form action="', $scripturl, '?action=quickmod;board=', $context['current_board'], '.', $context['start'], '" method="post" accept-charset="', $context['character_set'], '" class="clear" name="quickModForm" id="quickModForm">';
		...
		foreach ($context['topics'] as $topic)
		{
			...
			echo '
					...
					echo '			<div ', (!empty($topic['quick_mod']['modify']) ? 'id="topic_' . $topic['first_post']['id'] . '" onmouseout="mouse_on_div = 0;" onmouseover="mouse_on_div = 1;" ondblclick="modify_topic(\'' . $topic['id'] . '\', \'' . $topic['first_post']['id'] . '\', \'' . $context['session_id'] . '\', \'' . $context['session_var'] . '\');"' : ''), '>
									<strong><span id="msg_' . $topic['first_post']['id'] . '"><span class="mysubject">', $topic['first_post']['link'], '</span>' . (!$context['can_approve_posts'] && !$topic['approved'] ? '&nbsp;<em>(' . $txt['awaiting_approval'] . ')</em>' : ''), '</span></strong>';
			...
			elseif(!empty($settings['messagepreview']) && !empty($settings['messagepreview_last']))
				echo '
						<div class="smalltext" style="background: url('.$settings['images_url'].'/bubble.png) 3px 10px no-repeat; padding: 4px 0 0 25px; font-size: 85%;">&#171;', $topic['last_post']['preview'] , '&#187;</div>';
			...
			if(!empty($settings['avatarboards']))
				echo '
						<a href="', $topic['last_post']['href'], '"><img src="' . (!empty($avvy[$topic['last_post']['member']['id']]) ? $avvy[$topic['last_post']['member']['id']] : $settings['images_url'].'/none.png') . '" style="margin: 5px;float: right;width: 35px; height: 35px;" alt="" /></a>';
			else
				echo '<a href="', $topic['last_post']['href'], '"><img src="', $settings['images_url'], '/icons/last_post.gif" alt="', $txt['last_post'], '" title="', $txt['last_post'], '" /></a>';
